...
#library(ggblur)
# set wd
source("~/Dropbox/ottoLabNotes/cross_tol_data/bioscreen_functions.R")
library(viridis)
setwd("~/Dropbox/ottoLabNotes/cross_tol_data/raw_outcross_petite/")

# file with metadata of bioscreen wells
codes <- read.csv("~/Dropbox/ottoLabNotes/cross_tol_data/raw_outcross_petite/bioscreen20230418BIO1_codes.csv") # 
biosc2 <- read.csv("~/Dropbox/ottoLabNotes/cross_tol_data/raw_outcross_petite/20230418_BIO2.csv")
...
pdf("~/Dropbox/ottoLabNotes/cross_tol_data/figures/outcross_petite_metals.pdf", onefile = TRUE)
par(mfrow = c(10,4))
par(mar = c(0,0,0,0))
...
out_mydata6 <- out_mydata3 %>% 
  select(strain, E2,  maxSlope, anc_slope) %>% 
  arrange(strain, E2) %>% 
  group_by(strain, E2,  anc_slope) %>% 
  summarise(avg_slope = mean(maxSlope)) %>% 
  mutate(relfit1 = avg_slope - anc_slope)
...
out_pet=out_petite %>%
  ...
  ggplot(aes(x=E2,y=relfit1,fill=petite)) +
    ...
    geom_errorbar(data=out_summary1, aes(ymin = relfit1 - ci, ymax = relfit1 + ci), width=0.75, position=position_dodge2(width=0.75, preserve="single")) +
...
met_pet=mydata7 %>%
  ...
  ggplot(aes(x=E2,y=relfit1,fill=petite)) +
    ...
    stat_summary(fun=mean, geom="point", shape=21, size=4,
             position=position_dodge2(width=0.75, preserve="single")) +
    geom_errorbar(data=pet_summary1, aes(ymin = relfit1 - ci, ymax = relfit1 + ci), width=0.75, position=position_dodge2(width=0.75, preserve="single")) +
