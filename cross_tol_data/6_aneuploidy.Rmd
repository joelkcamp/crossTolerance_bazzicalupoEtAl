...
setwd("~/crossTolerance_bazzicalupoEtAl/cross_tol_data/depth/10_DepthAnalyses/")
require(stats)
files <-list.files()
#Reorder
...
num<-length(files)
#chromosomes<-c("chr1","chr2","chr3","chr4","chr5","chr6","chr7","chr8","chr9","chr10","chr11","chr12","chr13","chr14","chr15","chr16"); #wihtout mito
...
```{r }
pdf("~/crossTolerance_bazzicalupoEtAl/cross_tol_data/figures/coverage_chrom.pdf", onefile = TRUE)
#par(mfrow = c(10,4))
for (j in seq(0,110,4)) {
#quartz()
op <- par(mfcol=c(4,4), mar=c(2.1, 2.5, .2, .2), oma=c(0, 0, 1.5, 0))
for ( i in seq(1,16) ) { 
plot(locbychromo[[j+1]][chromosomes[i]][[1]],databychromo[[j+1]][chromosomes[i]][[1]],type="l",xlab="",ylab="",ylim=c(0,2.5),las=1,col="black")
points(locbychromo[[j+2]][chromosomes[i]][[1]],databychromo[[j+2]][chromosomes[i]][[1]],type="l",las=1,col="red")
points(locbychromo[[j+3]][chromosomes[i]][[1]],databychromo[[j+3]][chromosomes[i]][[1]],type="l",las=1,col="blue")
points(locbychromo[[j+4]][chromosomes[i]][[1]],databychromo[[j+4]][chromosomes[i]][[1]],type="l",las=1,col="green")
points(mean(centromerestart[[i]],centromereend[[i]]),2.1,type="p",xlab="",ylab="",las=1,col="black")
if (chromosomes[i] == "chr2") text(x=(311452+310565)/(2*10^6),y=2.22,"SCO1",cex=0.65)
if (chromosomes[i] == "chr3") text(x=(91324+92418)/(2*10^6),y=2.22,"LEU2",cex=0.65)
if (chromosomes[i] == "chr3") text(x=(198671+201177)/(2*10^6),y=2.22,"MAT",cex=0.65)
if (chromosomes[i] == "chr5") text(x=(116167+116970)/(2*10^6),y=2.22,"URA3",cex=0.65)
if (chromosomes[i] == "chr8") text(x=(212535+214718)/(2*10^6),y=2.16,"CUP1",cex=0.65)
if (chromosomes[i] == "chr8") text(x=(239098+241293)/(2*10^6),y=2.28,"ERG7",cex=0.65)
if (chromosomes[i] == "chr12") text(x=(732542+733876)/(2*10^6),y=2.22,"MET15",cex=0.65)
if (chromosomes[i] == "chr12") text(x=(253861+254958)/(2*10^6),y=2.22,"ERG3",cex=0.65)
if (chromosomes[i] == "chr12") text(x=(467569+460712)/(2*10^6),y=2.28,"RDN1",cex=0.65)
if (chromosomes[i] == "chr12") text(x=(467569+460712)/(2*10^6),y=2.16,"repeat",cex=0.65)
if (chromosomes[i] == "chr13") text(x=(252990+251839)/(2*10^6),y=2.22,"ERG6",cex=0.65)
if (chromosomes[i] == "chr13") text(x=(302485+300869)/(2*10^6),y=2.34,"ERG5",cex=0.65)
if (chromosomes[i] == "chr15") text(x=(721946+722608)/(2*10^6),y=2.22,"HIS3",cex=0.65)
legend("topright", chromosomes[[i]],bty="n")
...
for (j in seq(0,110)) {
for (i in seq(1,16) ) {
  something = mean(databychromo[[j+1]][chromosomes[i]][[1]])
  anotherthing = c(exptlist[j+1], chromosomes[i][[1]], something)
  chr_avg = rbind(chr_avg, anotherthing)
}
}
colnames(chr_avg) = c("strain","chr","cov")
write.csv(chr_avg, "~/crossTolerance_bazzicalupoEtAl/cross_tol_data/depth/chr_avg.csv")
...
i1 <- read.csv("~/crossTolerance_bazzicalupoEtAl/cross_tol_data/depth/whole_chrom_dups.csv")
...
mbm_rf <- mbm_rf %>% 
  mutate(ploidy = case_when(strain == "cdbm_37" ~ ">N",
                        strain == "cdbm_39" ~ ">N",
                        strain == "cubm_43" ~ ">N",
                        strain == "cubm_44" ~ ">N",
                        strain == "cubm_45" ~ ">N",
...
cbm_rf <- cbm_rf %>% 
  mutate(ploidy = case_when(strain == "CBM7" ~ ">N",
                        strain == "CBM29" ~ ">N",
                        strain == "CBM22" ~ ">N",
                        strain == "CBM21" ~ ">N",
                        strain == "CBM26" ~ ">N",
...
plot_mbm=mbm_rf %>%
  ...
  ggplot(aes(x=E2,y=relfit1,fill=ploidy)) +
    ...
    geom_errorbar(data=mbm_summary1, aes(ymin = relfit1 - ci, ymax = relfit1 + ci), width=0.75, position=position_dodge2(width=0.75, preserve="single")) +
...
plot_cbm=cbm_rf %>%
  ...
  ggplot(aes(x=E2,y=relfit1,fill=ploidy)) +
    ...
    stat_summary(fun=mean, geom="point", shape=21, size=4,
             position=position_dodge2(width=0.75, preserve="single")) +
    geom_errorbar(data=cbm_summary1, aes(ymin = relfit1 - ci, ymax = relfit1 + ci), width=0.75, position=position_dodge2(width=0.75, preserve="single")) +
    scale_fill_manual(values=petitePalette2) +
    #scale_fill_manual(legend_title_none, limits = c("Non-petite (n = 70)", "Petite (n =38)"),labels = c("Grande (n = 70)", "Petite (n = 38)"), values=petitePalette2) +
    labs(x="Test Environment", y="Relative Fitness") +
    theme_bw() +  geom_hline(yintercept=0, size=1)+
...
plot_cup=ggplot(df_cup, aes(x=CUP, y=relfit1, color=E1)) +
  ...
  scale_color_manual(legend_title3, values=relfitPalette)+
    labs(x="CUP scaled coverage", y="Relative Maximum Growth in Cu")
