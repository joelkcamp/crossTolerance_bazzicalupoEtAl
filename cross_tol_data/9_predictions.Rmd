...
cross.tol.combos$ORP <- ORPvis$diff[match(cross.tol.combos$combo,ORPvis$combo)] 
...
ct_orp =cross.tol.combos %>% 
  ggplot(aes(x=ORP,y=ct)) +
    ...
    geom_smooth(method="lm",linetype="dashed",color="black", se=FALSE) +
     theme_minimal() +
    labs(x="Predicted environmental distance", y="Cross-tolerance for pairs of metals")
ct_orp
...
ct_slope =cross.tol.combos %>% 
  ggplot(aes(x=all_slope,y=ct)) +
    ...
    geom_smooth(method="lm",linetype="dashed",color="black", se=FALSE) +
     theme_minimal() +
    labs(x="Predicted metal co-occurrence", y="Cross-tolerance for pairs of metals")
ct_slope
...
ct_prop =cross.tol.combos %>% 
  ggplot(aes(x=prop.syn,y=ct)) +
    ...
    geom_smooth(method="lm",linetype="dashed",color="black", se=FALSE) +
     theme_minimal() +
    labs(x="Predicted proportion of gene overlap", y="Cross-tolerance for pairs of metals")
ct_prop
...
```{r }
p1=plot_grid(
  orpmap, ct_orp, orplin, labels = c("A","B", "C"),
  ncol = 3
)
p2=plot_grid(propmap,ct_prop,  proplin, labels = c("D","E", "F"),
  ncol = 3
)
p3=plot_grid(usgsmap,ct_slope, usgslin, labels = c("G","H", "I"),
  ncol = 3
)

plot_grid(p1, p2, p3, ncol = 1)

ggsave("figures/prediction_grid.jpeg", width=15, height=12, units = "in", dpi=700)
```
