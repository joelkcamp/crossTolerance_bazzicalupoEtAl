...
mel_raw0 <- read.table("snps/full_genome.table", header=TRUE)
...
sites_to_check0 <- data.frame(matrix(nrow=1041,ncol=1)) # AB changed to 347 from 268
...
```{r}
annot <- read.csv("snps/sites_list_subset_ANNOTJoel_20230919.csv") %>% 
  rename("chr"="CHR") %>% 
  arrange(chr, POS)
...
mel_mut_annot <- left_join(mel_mut, annot, by = c('chr', 'POS')) %>% 
  ...
  select(-genotype) #%>% 
 # write.csv("snps/mutations_haploid.csv")
