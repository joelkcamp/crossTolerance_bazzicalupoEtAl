...
GTs_summary_RMlowcoverage <- GTs_by_site %>% 
  filter(genotype!=".") %>% 
  filter(!is.na(genotype)) %>% #ADDED second filter May22
  group_by(CHROM, POS) %>% 
  summarize(num_gt=n())
...
snpEff_intergenic_haploid <- filtered_snpEff %>% filter(effect!="LOW" & effect!="MODERATE" & effect!="HIGH") 
snpEff_intergenic_diploid %>% 
  write_tsv("snpEff_intergenic_haploid_19May2025.tsv")

snpEff_genic_haploid <- filtered_snpEff %>% filter(effect=="LOW" | effect=="MODERATE" | effect=="HIGH")
snpEff_genic_haploid %>% write_tsv("snpEff_genic_haploid_19May2025.tsv") #this has _ sites
