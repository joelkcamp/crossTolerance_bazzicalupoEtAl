...
```
```{r}
mutations <- read.csv("cross_tol_data/snps/mutations_pk.csv") 
```
...
regions_file <- mutations %>%
  ...
  mutate(
    CHROM = case_when(
      chr == "chr1" ~ "ref|NC_001133|", 
      chr == "chr2" ~ "ref|NC_001134|",
      chr == "chr3" ~ "ref|NC_001135|",
      chr == "chr4" ~ "ref|NC_001136|",
      chr == "chr5" ~ "ref|NC_001137|",
      chr == "chr6" ~ "ref|NC_001138|",
      chr == "chr7" ~ "ref|NC_001139|",
      chr == "chr8" ~ "ref|NC_001140|",
      chr == "chr9" ~ "ref|NC_001141|",
      chr == "chr10" ~ "ref|NC_001142|",
      chr == "chr11" ~ "ref|NC_001143|",
      chr == "chr12" ~ "ref|NC_001144|",
      chr == "chr13" ~ "ref|NC_001145|",
      chr == "chr14" ~ "ref|NC_001146|",
      chr == "chr15" ~ "ref|NC_001147|",
      chr == "chr16" ~ "ref|NC_001148|",
      chr == "chrmt" ~ "ref|NC_001224|"
    )
  ) %>%
  select(CHROM, POS) %>%
  arrange(CHROM, POS) %>% 
  distinct() %>% 
  write_tsv("regions_file.tsv")
```

```{r}
verified_snps <- read_tsv("regions_file_verified_ext.tsv") %>% 
  mutate(
    CHROM = case_when(
...
snpsFinal %>% 
  ...
  ggplot(aes(x=variant, fill=metal)) +
  geom_bar() +
  coord_flip() +
...
snpsFinal %>% 
  drop_na() %>% 
  filter(evo_line!="MnBM42" & evo_line!="MnBM14") %>% 
  ggplot(aes(x=metal, fill=variant)) +
  geom_bar() +
  coord_flip() +
...
snpsFinal %>% 
  drop_na() %>% 
  filter(evo_line!="MnBM42" & evo_line!="MnBM14") %>% 
  ggplot(aes(x=metal, fill=effect)) +
  geom_bar() +
  coord_flip() +
  scale_x_discrete(limits=c("Co","Mn","Cd","Zn","Cu","Ni"))
...
```
```{r}
snpsFINAL_Cd <- snpsFinal %>% filter(metal=="Cd") 
snpsFINAL_Co <- snpsFinal %>% filter(metal=="Co")
snpsFINAL_Cu <- snpsFinal %>% filter(metal=="Cu")
snpsFINAL_Mn <- snpsFinal %>% filter(metal=="Mn") %>% filter(evo_line!="MnBM42"&evo_line!="MnBM14")
snpsFINAL_Ni <- snpsFinal %>% filter(metal=="Ni")
snpsFINAL_Zn <- snpsFinal %>% filter(metal=="Zn")
```
```{r}
...
process_long <- read_tsv("cross_tol_data/process_slim_long_top15.tsv")
process_long[] <- lapply(process_long, function(x) gsub("YKL126W", "YPK1", x))
function_long <- read_tsv("cross_tol_data/snps/function_slim_long_final.tsv")
...
component_long <- read_tsv("cross_tol_data/snps/component_slim_long_final.tsv")
...
effect_term_compare %>% 
  ...
  theme_bw()
ggsave("GO_effect2.png", width=7, height=5, units = "in", dpi=700)
...
effect_term_compare %>% 
  ...
  theme_bw()
ggsave("GO_effect.png", width=10, height=7, units = "in", dpi=700)
```
...
strong_sel_reduced <- strong_selection_genes %>%
  ...
  filter(!duplicated(.[c("CHROM", "POS", "evo_line")])) %>% 
  write_csv("strong_sel_reduced.csv")
```
...
snps_count  
  ...
  theme_bw()
ggsave("number_mutations.png", width=7, height=5, units = "in")
```
```{r}
