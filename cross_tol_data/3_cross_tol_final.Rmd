...
pal2 <- c("#DFBF2B","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2","#3A9AB2")
...
abs_summary1 <- mydata6 %>% 
  #filter(E2!=".YPAD") %>% 
  Rmisc::summarySE(measurevar = "avg_slope", groupvars = c("E1", "E2"))
# maybe add a row with YPAD to have zeros in the figures?
...
stat_summary(fun=mean, geom="point", shape=21, size=4,
    ...
    geom_errorbar(data=diff_summary1, aes(ymin = relfitdiff - se, ymax = relfitdiff + se), width=0.75, position=position_dodge2(width=0.75, preserve="single")) +
...
stat_summary(fun=mean, geom="point", shape=21, size=4,
             position=position_dodge2(width=0.75, preserve="single")) +
    geom_errorbar(data=all_summary1, aes(ymin = relfit1 - se, ymax = relfit1 + se), width=0.75, position=position_dodge2(width=0.75, preserve="single")) +
...
test_env_abs=mydata6 %>% # run all_summary1 from previous chunk 
  #na.omit() %>% 
  #filter(E1!=".YPAD") %>% 
  ggplot(aes(x=E2,y=avg_slope,fill=E1)) +
    geom_point(alpha=0.2, size=0.5, position=position_dodge(width=0.75),aes(group=E1)) + 
      geom_hline(yintercept=0, size=1.5)+
stat_summary(fun=mean, geom="point", shape=21, size=4,
             position=position_dodge2(width=0.75, preserve="single")) +
    geom_errorbar(data=abs_summary1, aes(ymin = avg_slope - se, ymax = avg_slope + se), width=0.75, position=position_dodge2(width=0.75, preserve="single")) +
    scale_fill_manual(legend_metal, values=relfitPalette) +
    labs(x="Test Environment", y="Absolute\nFitness", title = "Absolute Growth Rate") +
    theme_bw() +
    theme(legend.title=element_text(size=rel(1))) +
      annotate("rect", xmin = 0.5, xmax = 1.5, ymin = -0.1, ymax = 0, alpha=0.6,fill = "#787878") +
  annotate("rect", xmin = 1.5, xmax = 2.5, ymin = -0.1, ymax = 0, alpha=0.6,fill = "#FF9A17") +
  annotate("rect", xmin = 2.5, xmax = 3.5, ymin = -0.1, ymax = 0, alpha=0.6,fill = "#3B83FF") +
  annotate("rect", xmin = 3.5, xmax = 4.5, ymin = -0.1, ymax = 0, alpha=0.6,fill = "#3EFF3B") +
  annotate("rect", xmin = 4.5, xmax = 5.5, ymin = -0.1, ymax = 0, alpha=0.6,fill = "#FD73FF") +
  annotate("rect", xmin = 5.5, xmax = 6.5, ymin = -0.1, ymax = 0, alpha=0.6,fill = "#FFF200") +
  annotate("rect", xmin = 6.5, xmax = 7.5, ymin = -0.1, ymax = 0, alpha=0.6,fill = "#A880FF") +
  geom_vline(xintercept = 1.5) + 
    geom_vline(xintercept = 2.5) + 
    geom_vline(xintercept = 3.5) + 
    geom_vline(xintercept = 4.5) + 
...
ggsave("figures/all_point_abs.jpeg", width=8, height=6, units = "in", dpi=1000)
```
...
cd_lines=E1cd_data %>% 
  ...
  theme_minimal()+
  ylim(-0.15, 0.18)+
    labs(x="Test Environment",y="Relative\nFitness", title = "Evolved in Cadmium")+
  theme(
...
co_lines=E1co_data %>% 
  ...
  theme_minimal()+
  ylim(-0.15, 0.18)+
    labs(x="Test Environment",y="Relative\nFitness", title = "Evolved in Cobalt")+
  theme(
  axis.line.x = element_line(size = 1, color = "black"),
  axis.text.x = element_text(size = 10, vjust = 1, face = "bold", color = "black"),
  axis.text.y = element_text(size = 10, face = "bold", color = "black" )
...
cu_lines=E1cu_data %>% 
  ...
  theme_minimal()+
  ylim(-0.15, 0.18)+
    labs(x="Test Environment",y="Relative\nFitness", title = "Evolved in Copper")+
  theme(
  axis.line.x = element_line(size = 1, color = "black"),
  axis.text.x = element_text(size = 10, vjust = 1, face = "bold", color = "black"),
  axis.text.y = element_text(size = 10, face = "bold", color = "black" )
...
mn_lines=E1mn_data %>% 
  ...
  theme_minimal()+
  ylim(-0.15, 0.18)+
    labs(x="Test Environment",y="Relative\nFitness", title = "Evolved in Manganese")+
  theme(
  axis.line.x = element_line(size = 1, color = "black"),
  axis.text.x = element_text(size = 10, vjust = 1, face = "bold", color = "black"),
  axis.text.y = element_text(size = 10, face = "bold", color = "black" )
...
ni_lines=E1ni_data %>% 
  ...
  theme_minimal()+
  ylim(-0.15, 0.18)+
    labs(x="Test Environment",y="Relative\nFitness", title = "Evolved in Nickel")+
  theme(
  axis.line.x = element_line(size = 1, color = "black"),
  axis.text.x = element_text(size = 10, vjust = 1, face = "bold", color = "black"),
  axis.text.y = element_text(size = 10, face = "bold", color = "black" )
...
profiles=plot_grid(cd_lines,co_lines,cu_lines,mn_lines,ni_lines,zn_lines,ncol = 3, align = "v",labels = c("D","E","F","G","H", "I"))
...
ggsave("figures/profiles_home1.jpeg", width=10, height=7, units = "in", dpi=700) #
```
...
cross_tol1=plot_grid(test_env_abs+ theme(legend.position = "none"),test_env_anc+ theme(legend.position = "none"),test_env_home+ theme(legend.position = "none"), profiles, ncol = 1, align = "h", labels = c("A", "B","C", ""), rel_heights = c(1,1,1,2))


#ggsave("figures/phenotypes1.jpeg", width=10, height=10, units = "in", dpi=700) # figure 2
#ggsave("figures/phenotypes_abs.jpeg", width=10, height=12, units = "in", dpi=700) # figure 2
ggsave("figures/phenotypes_all.jpeg", width=10, height=12, units = "in", dpi=700) # figure 2
```
...
ct_x$E1<- substr(ct_x$strain, 1, 2)
ct_x %>% group_by(E1) %>% summarise(mean(relfit1))
```

Add column with info about 9 genes with multiple SNPs

```{r}
ct_x = read.csv("mbm_intermediate_files/broad_cross_tolerance.csv")
ct_x$mutation = NA
...
0.042084356,
0.0188206964,
0.052254630, "gene with ≥3 SNP mutations","HUL5")
...
0.082120312,
0.0367253200,
0.101965835, "gene with ≥3 SNP mutations","VCX1")
mn14_2 = c("mnbm_14",
5,
0.0327898095,
0.082120312,
0.0367253200,
0.101965835, "gene with ≥3 SNP mutations","YGR130C")
cd30_1 = c("cdbm_30",
5,
0.0167164326,
0.024775732,
0.0110800441,
0.030763134, "gene with ≥3 SNP mutations","SFP1")
cd30_2 = c("cdbm_30",
5,
0.0167164326,
0.024775732,
0.0110800441,
0.030763134, "gene with ≥3 SNP mutations","VTC5")
co18 = c("cobm_18",
5,
0.0413293637,
0.044058443,
0.0197035347,
0.054705782, "gene with ≥3 SNP mutations","SIW14")
co7 = c("cobm_7",
5,
0.0970299424,
0.055952377,
0.0250226635,
0.069474052, "gene with ≥3 SNP mutations","PMA1")

ct_x = rbind(ct_x, mn42, mn14_1, mn14_2, cd30_1, cd30_2, co18, co7)


ct_x[is.na(ct_x)] <- "<3 SNP"

ct_x$relfit1 = as.numeric(ct_x$relfit1)
...
ct_x %>%
  na.omit() %>% 
  ggplot(aes(x=gene,y=relfit1,fill=gene, group=gene)) +
...
ct_VTC5 = ct_x %>%
  filter(gene==c("VTC5")) %>%
  rbind(ct_less3)
t.test(relfit1 ~ gene, data = ct_VTC5)
...
#test predictions against narrow cross tol
```{r}
predictions <- read.csv("predictions_TS12.csv")
prediction_lms <- cross.tol.combos %>% left_join(predictions)
